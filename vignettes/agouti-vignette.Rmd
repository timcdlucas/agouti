...
This vignette will demonstrate examples of the **AGOUti** package with
multi-resolution data of different formats. We advise readers work through the first example irrespective of the data structure in their own dataset before proceeding to the example most simliar to their data.
...
## Example 4 SpatialPolygonsDataFrame

In this example you have an outcome variable that relates to polygons (often geographical  administrative units such as counties in the UK). Your covariates are high-resolution rasters and you first need to extract the relevant information from your rasters along with the identifying polygon information.

Here we have two datasets 1) A shapefile (using data from the package `SpatialEpi` containing the polygons that are at the resolution of the outcome variable. 2) A raster or raster stack of high-resolution covariates. These raster covariates contain many pixels within a single polygon and each pixel has a value associated


```{r}
# two panel plot showing the datasets
## get plot data
polygons <- sf::st_as_sf(NYleukemia$spatial.polygon)
df <- cbind(polygons, NYleukemia$data)
p1 <- ggplot2::ggplot() + ggplot2::geom_sf(data = df, aes(fill = cases / population)) + ggplot2::scale_fill_viridis_c(lim = c(0, 0.003))
# get raster data (using annual mean temp from bioclim croped to the extent of the polyongs) -- need to add this to git. vignette/
...
This dataset can be transformed to the required format using the `as_disag()` function and specifying the number of lags to be considered as follows:
This function relies on parrallelExtract sourced from the disaggregation R package. Depending on the number of rasters in a stack and the size of the shapefile, this function may take a while to run.

```{r}
# disag_spdf <- agouti::as_disag(data=shapefile, rstack=rstack, response=wnv)
# class(disag_spdf)
# knitr::kable(head(disag_spdf), digits=2)
```
